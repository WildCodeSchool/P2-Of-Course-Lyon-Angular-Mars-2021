...
export class TravelListComponent implements OnInit {
  ...
  contOptions: string[] = this.voyageService
    .initVoyage()
    .map((x) => x._continent);
  ...
  onSubmit() {
    ...
    if (this.filtersForm.get('toLoc').value) {
      this.voyages = this.voyages.filter((x) =>
        x._name.includes(this.filtersForm.get('toLoc').value)
      );
    } else if (this.filtersForm.get('state').value) {
      this.voyages = this.voyages.filter((x) =>
        x._pays.includes(this.filtersForm.get('state').value)
      );
    } else if (this.filtersForm.get('continent').value) {
      this.voyages = this.voyages.filter((x) =>
        x._continent.includes(this.filtersForm.get('continent').value)
      );
    }
  }
